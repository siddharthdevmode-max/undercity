...
export const getCrimes = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(500).json({ message: "Failed to fetch crimes", error: error.message });
  } finally {
    client.release();
  }
};
...
// NOW WITH FULL ANTI-CHEAT INTEGRATION
// ============================================================
...
export const attemptCrime = async (req: Request, res: Response) => {
  ...
  try {
    const firebaseUser = (req as any).firebaseUser;
    const firebaseUid = firebaseUser?.uid;
    const trustInfo = (req as any).trustInfo || { isShadowBanned: false, trustScore: 100 };
    ...
    // 🛡️ FIRE & FORGET: Record fingerprint + analyze behavior (don't block on these)
    recordFingerprint(firebaseUid, ipAddress, userAgent).catch(() => {});
    ...
    if (isFutureDate(user.federal_jail_until)) {
      ...
      return res.status(423).json({
        ...
        federalJailUntil: user.federal_jail_until,
        secondsRemaining: Math.ceil((new Date(user.federal_jail_until).getTime() - Date.now()) / 1000),
      });
    }
    if (isFutureDate(user.jail_until)) {
      ...
      return res.status(423).json({
        ...
        jailUntil: user.jail_until,
        secondsRemaining: Math.ceil((new Date(user.jail_until).getTime() - Date.now()) / 1000),
      });
    }
    ...
    if (toNumber(user.level) < crime.unlock_level) {
      await client.query("ROLLBACK");
      return res.status(403).json({ message: `You need to be level ${crime.unlock_level} to attempt this crime.` });
    }
    ...
    const specialResult = await client.query(
      `SELECT cs.* FROM crime_specials cs
       WHERE cs.crime_id = $1 AND cs.is_active = TRUE
         ...
         AND NOT EXISTS (SELECT 1 FROM user_crime_specials ucs WHERE ucs.user_id = $3 AND ucs.crime_special_id = cs.id)
       ORDER BY RANDOM() LIMIT 1`,
      [crime.id, progress.crime_level, user.id]
    );
    const availableSpecial = specialResult.rows.length > 0 ? parseSpecial(specialResult.rows[0]) : null;

    const playerLevel = toNumber(user.level);
    const maxLife = calcMaxLife(playerLevel);

    let outcomeResult = resolveCrimeOutcome(crime, progress, availableSpecial, toNumber(user.money), maxLife);

    // 🔥 SHADOW PUNISHMENT - The fun part
    if (trustInfo.isShadowBanned) {
      console.log(`👻 Shadow punishing user ${firebaseUid.substring(0, 8)}...`);
      outcomeResult = applyShadowPunishment(outcomeResult, trustInfo.trustScore);
    }

    const updatedNerve = Math.max(0, toNumber(user.nerve) - crime.nerve_cost);
    const updatedMoney = Math.max(0, toNumber(user.money) - outcomeResult.money_loss + outcomeResult.reward_money);
    const updatedPoints = Math.max(0, toNumber(user.points) + outcomeResult.reward_points);
    const updatedLife = Math.max(1, toNumber(user.life) - outcomeResult.life_loss);
    const updatedCrimeXp = Math.max(0, progress.crime_xp + outcomeResult.xp_gained - outcomeResult.xp_lost);
    const updatedCrimeLevel = calcCrimeLevel(updatedCrimeXp);
    const updatedHiddenCpl = Math.max(0, progress.hidden_cpl + outcomeResult.cpl_change);

    const attempts = progress.attempts + 1;
    const successes = progress.successes + ((outcomeResult.outcome === "success" || outcomeResult.outcome === "special") ? 1 : 0);
    const failures = progress.failures + (outcomeResult.outcome === "fail" ? 1 : 0);
    const critFailures = progress.crit_failures + (outcomeResult.outcome === "crit_fail" ? 1 : 0);
    ...
    await client.query(
      ...
      [updatedCrimeXp, updatedCrimeLevel, updatedHiddenCpl, attempts, successes, failures, critFailures, updatedSpecialsFoundCount, user.id, crime.id]
    );

    let jailUntil: Date | null = user.jail_until ? new Date(user.jail_until) : null;
    let federalJailUntil: Date | null = user.federal_jail_until ? new Date(user.federal_jail_until) : null;
    ...
    await client.query(
      `UPDATE users
       ...
       WHERE id = $9`,
      [updatedMoney, updatedPoints, finalNerve, updatedMaxNerve, updatedLife, maxLife, jailUntil, federalJailUntil, user.id]
    );
    ...
    return res.json({
      ...
      crime: { id: crime.id, key: crime.crime_key, name: crime.name, tier: crime.tier, nerveCost: crime.nerve_cost, isFederal: crime.is_federal },
      rewards: { money: outcomeResult.reward_money, points: outcomeResult.reward_points, xpGained: outcomeResult.xp_gained },
      penalties: {
        moneyLost: outcomeResult.money_loss, lifeLost: outcomeResult.life_loss, xpLost: outcomeResult.xp_lost,
        jailSeconds: outcomeResult.jail_seconds,
        jailType: outcomeResult.jail_seconds > 0 ? (crime.is_federal ? "federal" : "normal") : null,
      },
      special: outcomeResult.special ? {
        id: outcomeResult.special.id, title: outcomeResult.special.title, description: outcomeResult.special.description,
        rewardMoney: outcomeResult.special.reward_money, rewardPoints: outcomeResult.special.reward_points,
        wasNewlyDiscovered: specialDiscovered,
      } : null,
      progress: { crimeXp: updatedCrimeXp, crimeLevel: updatedCrimeLevel, attempts, successes, failures, critFailures, specialsFoundCount: updatedSpecialsFoundCount },
      user: {
        money: updatedMoney, points: updatedPoints, nerve: finalNerve, maxNerve: updatedMaxNerve,
        life: updatedLife, maxLife, jailUntil: jailUntil ? jailUntil.toISOString() : null,
    ...
    return res.status(500).json({ message: "Crime attempt failed", error: error.message });
  } finally {
    client.release();
  }
};
