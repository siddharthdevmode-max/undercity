...
// Manual shadow ban without hard banning the user
// ============================================================
router.post(
  "/shadow-ban/:uid",
  verifyFirebaseToken,
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await pool.query(
      `UPDATE users
       SET trust_score = LEAST(COALESCE(trust_score, 100), 19),
           ...
           is_hard_banned = FALSE,
           last_flag_reason = 'Manual admin shadow ban',
    ...
    logger.warn(`⚠️ Admin shadow-banned user: ${uid.substring(0, 8)}...`);
    res.json({
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [violationsResult, trustRecoveryResult, fingerprintsResult, linkedAccountsResult, crimeProgressResult] =
      await Promise.all([
        pool.query(
          `SELECT violation_type, severity, details, ip_address, user_agent, created_at
           FROM uac_violations
...
// Latest earnings velocity detections
// ============================================================
router.get(
  "/earnings-anomalies",
  verifyFirebaseToken,
