...
import { marketListingSchema } from "../utils/schemas";
...
router.post("/buy/:listingId", verifyFirebaseToken, marketLimiter, asyncHandler(async (req, res) => {
  const uid = req.firebaseUser!.uid;
...
router.delete("/listing/:listingId", verifyFirebaseToken, marketLimiter, asyncHandler(async (req, res) => {
  const uid = req.firebaseUser!.uid;
  const listingId = parseInt(req.params["listingId"] ?? "0", 10);
  const userR = await pool.query<{ id: number }>(
    `SELECT id FROM users WHERE firebase_uid = $1 AND deleted_at IS NULL LIMIT 1`, [uid]
